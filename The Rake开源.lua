...
if game.GameId == 847722000 then
	...
	local Rayfield = loadstring(game:HttpGet('https://pastebin.com/raw/f1RG4AsU"'))()

	local Window = Rayfield:CreateWindow({
		Name = "OVOV",
		LoadingTitle = "OVOV",
		LoadingSubtitle = "OVOV",
		ConfigurationSaving = {
			Enabled = true,
			FolderName = "TheRake",
			FileName = "OVOV"
	...
	local SettingsTab = Window:CreateTab("设置", 132680679646084)
						...
						if not v.Character then
							...
							if v.Character:FindFirstChild("Head") then
								if v.Character:GetAttribute("PlayerESP") == true then
									...
									ESP.Size = 18
									ESP.Font = 18
									ESP.Text = v.Name.." [步数: "..tostring(v.DistanceTravelled.Value).."]"
									ESP.Visible = false
